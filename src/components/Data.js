      ...
      <div className="split right-section">
        ...
        {result && (
          ...
          <div>
            {result && (
              <div>
                <p>Accuracy: { result.train.accuracy}</p>
                <p>Precision: { result.train.precision}</p>
                <p>F1 Score: { result.train.f1_score}</p>
                <p>Confusion Matrix: {JSON.stringify( result.train.confusion_matrix)}</p>
                <p>Test Result</p>
                <p>Accuracy: {testButton? result.test.accuracy :null}</p>
                <p>Precision: {testButton ? result.test.precision : null}</p>
                <p>F1 Score: {testButton ? result.test.f1_score : null}</p>
                <p>Confusion Matrix: {JSON.stringify(testButton ? result.test.confusion_matrix : null)}</p>
                <img src={`http://127.0.0.1:8000/learning-curve`} alt="Learning Curve" />
              </div>
            )}
          </div>
